...
impl<Smp: Float> ModFilt<Smp> {
    ...
    pub fn process(&mut self, input: &[Smp], env: &[Smp], note: &[Smp], params: ModFiltParams<Smp>) -> &[Smp] {
        let numsamples = *[input.len(), env.len(), note.len(), params.len()].iter().min().unwrap();
        ...
        let filt_out = self.filter.process(&input,
            FiltParams { cutoff: &self.outbuf[0..numsamples], resonance: params.resonance });
        //now mix the outputs
        for i in 0..numsamples {
            self.outbuf[i] =
                (params.low_mix[i] * filt_out.low[i]) +
...
impl ModFiltFxP {
    ...
    pub fn process(&mut self, input: &[SampleFxP], env: &[ScalarFxP], note: &[NoteFxP], params: ModFiltParamsFxP) -> &[SampleFxP] {
        let numsamples = *[input.len(), env.len(), note.len(), params.len()].iter().min().unwrap();
        for i in 0..numsamples {
            // reinterpret the env_mod as a number from 0 to NOTE_MAX instead of 0 to 1,
            // then multiply by the envelope output:
            let envmod = NoteFxP::from_num(
                NoteFxP::from_bits(params.env_mod[i].to_bits())
                .wide_mul(env[i]));
            ...
            self.modbuf[i] = params.cutoff[i].saturating_add(envmod).saturating_add(kbdmod);
        }
        //calculate filter output
        let filt_out = self.filter.process(&input,
            FiltParamsFxP { cutoff: &self.modbuf[0..numsamples], resonance: params.resonance });
        //now mix the outputs
        for i in 0..numsamples {
            ...
            self.outbuf[i] = SampleFxP::from_num(
                low.saturating_add(band).saturating_add(high));
        }
        &self.outbuf[0..numsamples]
    }
}